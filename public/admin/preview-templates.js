/* ==========================================================
   sveltia CMS プレビューテンプレート集
   hyperscript(h) + createClass のみで書かれています（ビルド不要）。
   対象外（登録なし・標準プレビューのまま）: site / navigation / related-links / news-page-meta
   ========================================================== */
(function () {
  // Sveltia CMSはCMS本体の初期化がこのscriptタグの実行より遅れることがあるため、
  // h / createClass が生えるまで最大2秒リトライしてから処理を始める。
  function boot(retriesLeft) {
    ...
    if (!h || !createClass) {
      ...
      console.error(
        '[preview] h / createClass が見つかりません。CMS本体（Sveltia CMS）の読み込み順序、' +
          'またはバージョンがカスタムプレビューテンプレートAPIに対応しているか確認してください。',
        window.CMS,
      );
      return;
    }

    registerAll(h, createClass);
  }

  boot(20);

  function registerAll(h, createClass) {
    // ---------- 共通ヘルパー ----------
    function toJS(v) {
      return v && typeof v.toJS === 'function' ? v.toJS() : v;
    }
    // ...(以下、既存の Eyebrow / Lines / ImgTag / PageTitle / QuickNavBadges /
    //      LabelValueRow / Card / Wrapper / 各 CMS.registerPreviewTemplate(...) 呼び出しを
    //      すべてこの registerAll 関数の中にそのまま移すだけでOK)...
                ...
                return Card(
                  [
                    ...
                    h(
                      ...
                      {
                        key: 'n',
                        style: { fontWeight: 600, marginBottom: '4px', color: COLOR.ink },
                      },
                      get(item, 'name'),
                    ),
                    h(
        ...
        render: function () {
          ...
          return Wrapper(
            ...
            h(
              ...
              '※ 客室タイプ（和室・洋室など）本体は「客室一覧」コレクションで編集してください。',
            ),
            ...
            // ── ご滞在案内バー（チェックイン/アウト・支払い・キャンセル規定） ──
            stayNotice &&
              h(
                'div',
                {
                ...
                [
                  {
                    label: get(stayNotice, 'checkInLabel'),
                    value: get(stayNotice, 'checkInValue'),
                  },
                  {
                    label: get(stayNotice, 'checkOutLabel'),
                    value: get(stayNotice, 'checkOutValue'),
                  },
                  {
                    label: get(stayNotice, 'paymentLabel'),
                    value: get(stayNotice, 'paymentValue'),
                  },
                  {
                    label: get(stayNotice, 'cancellationLabel'),
                    value: get(stayNotice, 'cancellationValue'),
                  },
                ].map(function (item, i) {
                  return h(
                    'div',
                    ...
                    h('p', { style: { fontSize: '0.84rem', color: COLOR.ink } }, item.value),
                  );
                }),
              ),
            ...
            commonAmenities.length > 0 &&
              h(
                ...
                Eyebrow(amenitiesIcon, amenitiesEyebrow, amenitiesHeading),
                h(
                  'div',
            ...
            firstFloorNotice &&
              h(
                ...
                arr(get(firstFloorNotice, 'items')).map(function (it, i) {
                  return h('p', { key: i, style: { marginBottom: '2px' } }, '※ ' + it);
                }),
              ),
    ...
    CMS.registerPreviewTemplate(
      ...
      createClass({
        render: function () {
          var data = this.props.entry.get('data');
          var getAsset = this.props.getAsset;
          var basicInfo = arr(data.get('basicInfo'));
          var floors = arr(data.get('floors'));
          var image = data.get('image');

          return Wrapper(
            Eyebrow(data.get('icon'), data.get('eyebrow'), data.get('name')),

            // 代表画像
            h(
              'div',
              { style: { height: '200px', overflow: 'hidden', marginBottom: '16px' } },
              ImgTag(getAsset, get(image, 'src'), get(image, 'alt') || get(image, 'label')),
            ),

            // 説明文
            Lines(data.get('description'), { marginBottom: '16px' }),
              ...
              h(
                'div',
                {
                  style: {
                    ...
                    padding: '12px 16px',
                    background: COLOR.panel,
                    border: '1px solid ' + COLOR.bengara + '30',
                  },
                },
                h(
                  'p',
                  { style: { fontSize: '0.72rem', color: COLOR.bengara, marginBottom: '4px' } },
                  data.get('priceLabel'),
                ),
                h(
                  'p',
            ...
            floors.map(function (floor, i) {
              var floorImages = arr(get(floor, 'images'));
              var badges = arr(get(floor, 'badges'));
              return h(
                'div',
                {
                  key: i,
                ...
                h(
                  ...
                  get(floor, 'label') + (get(floor, 'isFirstFloor') ? '（1階）' : ''),
                ),
                h(
                  'p',
                  { style: { fontSize: '0.72rem', color: COLOR.inkFaint, marginBottom: '8px' } },
                  '予約サイト表示名: ' + get(floor, 'reservationName'),
                ),
                badges.length > 0 &&
                  h(
                    'div',
                    ...
                    badges.map(function (badge, bi) {
                      return h(
                        'span',
                        {
                          ...
                          style: {
                            fontSize: '0.68rem',
                            border: '1px solid ' + COLOR.bengara + '60',
                            color: COLOR.bengara,
                            borderRadius: '2px',
                            padding: '2px 8px',
                          },
                ...
                floorImages.length > 0 &&
                  h(
                    'div',
                    {
                      style: {
                        display: 'grid',
                        gridTemplateColumns: 'repeat(' + Math.min(floorImages.length, 3) + ', 1fr)',
                        gap: '8px',
                        marginBottom: '10px',
                      },
                    },
                    floorImages.map(function (img, ii) {
                      return h(
                        'div',
                        { key: ii, style: { height: '90px', overflow: 'hidden' } },
                        ImgTag(getAsset, get(img, 'src'), get(img, 'alt') || get(img, 'label')),
                      );
                    }),
                  ),
      ...
      return Card(
        [
          ...
          h(
            ...
            {
              key: 'note',
              style: { fontSize: '0.74rem', color: COLOR.inkFaint, marginTop: '8px' },
            },
                ...
                return Card(
                  [
                    ...
                    h(
                      ...
                      {
                        key: 'h',
                        style: { fontWeight: 600, color: COLOR.ink, marginBottom: '6px' },
                      },
                      get(v, 'heading'),
                    ),
                    ...
                    [
                      ...
                      get(item, 'reservationMethod') &&
                        h(
                          'p',
                          { key: 'rm' },
                          data.get('reservationMethodLabel') +
                            ': ' +
                            get(item, 'reservationMethod'),
                        ),
                      get(item, 'location') &&
